...
def main():
    ...
    try:
        ...
        write_clusters(closed_clusters, '_closed.csv', ctype='close')
        write_clusters(open_clusters, '_open.csv', ctype='open')
...
def write_clusters(clusters: Dict, filename: str, ctype: str) -> None:
    """Write clusters to a CSV file with calculated profit metrics, days, and average days for closed clusters."""
    headers = ['Account', 'Ticker', 'Date', 'LastDate', 'Quantity', 'Price', 'Commission', 'Fees',
                ...
                for rundate in sorted(clusters[account].keys()):
                    for ticker in sorted(clusters[account][rundate].keys()):
                        ...
                        for cluster in clusters[account][rundate][ticker]:
                            cluster_profit, cluster_investment, days = write_cluster(writer, cluster, filename.endswith('_closed.csv'), ctype=ctype)
...
def write_cluster(writer: csv.writer, cluster: List[Dict[str, str]], calculate_profit: bool, ctype: str) -> Tuple[float, float, int]:
    ...
    for row in cluster:
        ...
        writer.writerow(['', '', datefmt(rundate), datefmt(lastdate), quantity, price, commission, fees, amount, '', '', '', '', ''])
